...
install_monolith_tools() {
    ...
    if ${DRY_RUN}; then
        info "[DRY RUN] Would build and install mnctl, mnpkg, mntui, mnweb"
        return
    fi

    local src
    src="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd 2>/dev/null)"

    # When run via curl | bash, script_dir is /dev or tmp — clone the repo
    if [[ ! -f "${src}/Cargo.toml" ]]; then
        info "Local source not found — cloning from GitHub..."
        local clone_dest
        clone_dest="$(mktemp -d)"
        git clone --depth 1 "${REPO_URL}" "${clone_dest}" 2>&1 | tee -a "${LOG_FILE}"
        src="${clone_dest}"
    fi

    if [[ ! -f "${src}/Cargo.toml" ]]; then
        die "Cannot find Monolith source. Clone manually: git clone ${REPO_URL}"
    fi
    ...
    install -Dm755 target/release/mnweb /usr/bin/mnweb 2>/dev/null || true
