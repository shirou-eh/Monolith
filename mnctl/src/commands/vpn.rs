...
enum PeerCommand {
    ...
    /// Generate a QR code for a WireGuard peer config
    Qr {
        /// Peer name
        name: String,
    },
    ...
    pub async fn run(self) -> Result<()> {
        match self.command {
            ...
            VpnCommand::Peer(args) => match args.command {
                ...
                PeerCommand::Qr { name } => vpn_peer_qr(&name),
            },
        }
    }
}
...
fn add_peer(tunnel: &str, pubkey: &str) -> Result<()> {
    ...
    let _ = Command::new("wg")
        .args(["set", tunnel, "peer", pubkey, "allowed-ips", allowed_ips])
        .status();
...
fn vpn_peer_qr(name: &str) -> Result<()> {
    let peer_cfg = format!("/etc/wireguard/peers/{name}.conf");
    if !std::path::Path::new(&peer_cfg).exists() {
        anyhow::bail!("peer config not found: {peer_cfg}");
    }

    let content = std::fs::read_to_string(&peer_cfg)?;
    println!("{} WireGuard peer '{}'", "≡".blue().bold(), name);
    println!("  {} Generate QR with qrencode:", "→".blue());
    println!();

    // Try to render QR directly if qrencode is available
    let has_qrencode = std::process::Command::new("which")
        .arg("qrencode")
        .output()
    ...
    if has_qrencode {
        let output = std::process::Command::new("qrencode")
            .args(["-t", "ANSIUTF8", "-r", &peer_cfg])
            .output()
            .context("qrencode failed")?;
        print!("{}", String::from_utf8_lossy(&output.stdout));
    } else {
        println!("    # Install qrencode to generate QR in terminal:");
    ...
    println!();
    println!("{} Config preview:", "●".cyan());
    for line in content.lines() {
        if line.contains("PrivateKey") {
            println!("  {} PrivateKey = {}", "…".dimmed(), "[hidden]".dimmed());
        } else {
            println!("  {line}");
        }
    }
