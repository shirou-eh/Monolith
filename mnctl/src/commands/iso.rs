...
enum IsoCommand {
    ...
    Build {
        ...
        /// Path to monolith.toml to bake into the ISO
        #[arg(long)]
        bake_config: Option<String>,
    },
...
fn build(
    ...
    if let Some(config_path) = &bake_config {
        let config_content = std::fs::read_to_string(config_path)
            .context(format!("failed to read bake config: {config_path}"))?;

        let workdir = std::env::temp_dir().join("monolith-iso-build");
        std::fs::create_dir_all(&workdir)?;
        let iso_overlay = format!("{}/overlay/etc/monolith", workdir.display());
        std::fs::create_dir_all(&iso_overlay)?;
        std::fs::write(format!("{iso_overlay}/monolith.toml"), &config_content)?;
        println!("  {} Baked config: {}", "●".green(), config_path);
    }
