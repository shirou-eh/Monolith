...
fn service_logs(name: &str, lines: u32, follow: bool, since: Option<&str>) -> Result<()> {
    ...
    let mut cmd_args: Vec<&str> = vec!["-u", name, "-n", &lines_str];
    if let Some(s) = since {
        cmd_args.push("--since");
        cmd_args.push(s);
    }
    ...
    if follow {
        // Exec into journalctl directly so terminal state is managed by
        // journalctl itself — avoids leaving the shell in raw mode when
        // the user presses q to quit follow mode.
        let err = Command::new("journalctl").args(&cmd_args).arg("-f").exec();
        anyhow::bail!("failed to exec journalctl: {err}");
    }

    cmd_args.push("--no-pager");
