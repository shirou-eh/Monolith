...
fn list_proxies() -> Result<()> {
    ...
    for entry in std::fs::read_dir(path).context("failed to read nginx sites")? {
        ...
        let domain = name.trim_end_matches(".conf");
        ...
        // Check TLS certificate expiry for this domain
        ...
        let tls_expiry = if std::path::Path::new(&cert_path).exists() {
            ...
            match expiry_output {
                Ok(o) if o.status.success() => {
                    let enddate = String::from_utf8_lossy(&o.stdout);
                    // Parse "notAfter=May 31 12:00:00 2026 GMT"
                    if let Some(date_str) = enddate.strip_prefix("notAfter=") {
        ...
        let (glyph, tls_color) = if tls_expiry == "—" {
            ("●".green(), tls_expiry.dimmed())
        } else if tls_expiry == "unknown" {
            ("●".yellow(), tls_expiry.yellow())
        } else {
            ("●".green(), tls_expiry.green())
        };
