...
enum ConfigCommand {
    ...
    Validate,
    /// Show a unified diff between the live config and built-in defaults
    Diff {
        /// Strip ANSI colour output (for piping into patch/diff)
        #[arg(long)]
        color_never: bool,
    },
    /// Auto-migrate configuration when schema changes
    Migrate,
}
...
impl ConfigArgs {
    pub async fn run(self) -> Result<()> {
        match self.command {
            ...
            ConfigCommand::Diff { color_never } => config_diff(color_never),
...
/// Show a unified diff between the live config and compiled-in defaults.
fn config_diff(color_never: bool) -> Result<()> {
    let default_config = include_str!("../../config_default.toml");

    let live_path = std::path::Path::new(CONFIG_PATH);
    let live_config = if live_path.exists() {
        std::fs::read_to_string(live_path).context("failed to read config")?
    } else {
        String::new()
    };

    let diff = TextDiff::from_lines(default_config, &live_config);
    ...
    if color_never {
        ...
        for change in diff.iter_all_changes() {
            ...
            print!("{}", format!("{}{}", sign, change.value()).color(style));
        }
    }
...
fn config_migrate() -> Result<()> {
    let config_paths = vec![
        ...
        "/etc/monolith/mnweb.toml",
        "/etc/monolith/cluster.toml",
    ];

    let schema_version = env!("CARGO_PKG_VERSION");
    let marker_file = "/var/lib/monolith/.schema-version";

    let current_version = std::fs::read_to_string(marker_file).unwrap_or_default();
    let current_ver = current_version.trim();

    if current_ver == schema_version {
        println!("{} Schema is up-to-date (v{schema_version})", "●".green());
        return Ok(());
    }
    ...
    for path in &config_paths {
        ...
        let backup = format!("{path}.bak-{schema_version}");
        std::fs::write(&backup, &content)?;

        // Perform any needed migrations based on version
        ...
        // Migration: v1.1.x → v1.2.0: add [secrets] section
        if !content.contains("[secrets]") {
            updated.push_str("\n[secrets]\n# Added by schema migration v1.2.0\nbackend = \"age\"\nencrypted_dir = \"/var/lib/monolith/secrets\"\n");
            migrated += 1;
        }

        if updated != content {
            std::fs::write(path, &updated)?;
            println!("  {} {} — migrated (backup: {})", "✓".green(), path, backup);
        } else {
            println!("  {} {} — no changes needed", "●".cyan(), path);
        }
    }
